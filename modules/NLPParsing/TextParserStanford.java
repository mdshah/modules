	...
	public static List<Sentence> parseDesc(String desc){
		...
		for(CoreMap sentence: document.get(SentencesAnnotation.class)) { 
			for(CoreLabel token: sentence.get(TokensAnnotation.class)) {            
				...
				posTags.add(token.get(PartOfSpeechAnnotation.class).toLowerCase());
				chunks.add(token.get(ChunkAnnotation.class).toLowerCase());
				ner.add(token.get(NamedEntityTagAnnotation.class).toLowerCase());
